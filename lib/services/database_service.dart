...
class DatabaseService {
  ...
  Future<void> updateAccount(Account a) async => await db
  ...
  Future<void> updateCategory(AppCategory c) async => await db
  ...
  Future<void> insertTag(Tag t) async => await db.insert('tags', t.toMap(),
      conflictAlgorithm: ConflictAlgorithm.replace);
  ...
  Future<List<AppTransaction>> getTransactions({
    ...
    if (accountId != null) {
      where += ' AND (accountId = ? OR toAccountId = ?)';
  ...
  Future<void> updateTransaction(
      AppTransaction oldT, AppTransaction newT) async {
    await _updateAccountBalance(oldT, isDelete: true);
    await db.update('transactions', newT.toMap(),
  ...
  Future<void> _updateAccountBalance(AppTransaction t,
    ...
    if (t.type == TransactionType.income) {
      ...
      if (acc != null) {
        await updateAccount(
            acc.copyWith(balance: acc.balance + (t.amount * multiplier)));
      }
      ...
      if (from != null) {
        await updateAccount(
            from.copyWith(balance: from.balance - (t.amount * multiplier)));
      }
      ...
      if (to != null) {
        if (to.type == 'crypto') {
          ...
          if (coinAmount != null) {
            await updateAccount(
                to.copyWith(balance: to.balance + (coinAmount * multiplier)));
          } else {
            debugPrint(
                '⚠️ Transfer ke crypto tanpa metadata coin amount — saldo tidak diubah');
          }
        } else {
          // Rekening biasa: tambah IDR normal
          await updateAccount(
              to.copyWith(balance: to.balance + (t.amount * multiplier)));
        }
      }
    }
  }
